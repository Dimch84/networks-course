...
2. Какие языки (если есть) ваш браузер может принимать? В захваченном сеансе какую еще информацию (если есть) браузер предоставляет серверу относительно пользователя/браузера?
   - русский, английский (`Accept-Language`)
   - браузер и платформа, на которой он запущен (с версиями)
...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - да
   - `Content-Length: 371`, да и, в целом, данные о контенте
3. Теперь проверьте содержимое второго HTTP-запроса GET (из вашего браузера на сторону сервера). Видите ли вы строку «IF-MODIFIED-SINCE» в HTTP GET? Если да, то какая
   информация следует за заголовком «IF-MODIFIED-SINCE»?
   - да, после заголовка следует дата, равная Last-Modified первого ответа сервера
...
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или они были загружены с веб-сайтов параллельно? Объясните.
   - Я думаю, что браузер загрузил изображения параллельно, так как сначала были напралены оба get запроса на картинки, а только потом был получен ответ
...
2. Когда ваш браузер отправляет сообщение HTTP GET во второй раз, какое новое поле включается в сообщение HTTP GET?
   - `Authorization`, включающий в себя `Credentials`
